...
он его не собирает. Для LuCI-пакета это значит: CSS уже собран, а `.po` уже скомпилированы в `.lmo`.
owfeed компилировать их не будет, и это не лень:

- Базовое имя каталога — packaging-решение, а не выводимое. `luci-theme-footstrap` намеренно называет
  свои каталоги `footstrap-theme.<lang>.lmo`, а не `footstrap.<lang>.lmo`, чтобы не столкнуться с
  файлом из пакета `luci-i18n-footstrap-<lang>`, который всё ещё принадлежит старому роутеру. Тот же
  путь = конфликт файлов = apk отказывает ровно в том апгрейде, ради которого всё делалось. Угадав
  имя, owfeed сломал бы именно этот случай.
- `po2lmo` живёт в `hostpkg/bin` и появляется только после host-сборки `luci-base`; в SDK-тарболе
  его нет. Тащить его к себе — значит стать вторым `luci.mk`, что дизайн запрещает выше.

Вместо этого owfeed **отказывается паковать исходники**: `.po`/`.pot`, `.scss`/`.less`,
`node_modules/`, `.git`, `.DS_Store` в payload — ошибка с указанием файла и причины. Ошибка
«`files:` показывает на дерево исходников» становится громкой, а не тихой: без этой проверки пакет
собирается, ставится, выглядит целым — и не имеет переводов вообще.

Поэтому `luci-theme-footstrap` через mkpkg-путь собирается, если мейнтейнер подготовил rootfs
(его Makefile уже умеет всё нужное). Полная сборка темы из исходников — это SDK-путь, v0.2.
