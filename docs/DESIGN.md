...
Вывод по релиз-линии, по убыванию предпочтения:

1. `https://downloads.openwrt.org/releases/<point>/.overview.json` → объединение `arch_packages`
   по всем профилям. Один запрос, авторитетно.
2. Пер-таргетные `profiles.json` (поле `arch_packages`).
3. Листинг каталога `…/releases/<point>/packages/` — каждая поддиректория есть арка.

Это само по себе даёт `riscv64_generic` (не `riscv64_riscv64`) на 25.12, `powerpc_8548` (не `8540`)
на 23.05+, и само опускает `mips_4kec`/`mipsel_24kc_24kf` на 25.12. Инлайновые тернарники meshtastic
и 19 строк `exclude:` исчезают: «переименованная» арка — это просто арка, которой больше нет в источнике.

Кэш: `~/.cache/owfeed/arches/<point>.json` с ETag-ревалидацией. Результат пишется в `owfeed.lock`
вместе с URL и sha256 источника. В CI дефолт `--frozen-lock`: расхождение с локом → exit 9 с дифом
и командой `owfeed lock --update`. Новая арка в апстриме никогда не меняет тихо то, что вы публикуете.
...
v0.1 превращает это в ~2 секунды и демонстрируется на `luci-theme-footstrap` в первый же день.

**v0.2 — «работает для реального мульти-пакетного фида».** SDK-сборки через `gh-action-sdk` с
матрицей из лок-файла. GitHub Action + reusable workflow. `collect` (двухуровневая схема, арка из
метадаты). Подпись каждого пакета + `smoke`, доказывающий утверждение про LuCI Upload.
Keyring-пакет. Цели R2 и rsync. `gc` с зелёным гейтом. `verify` с 511/512/513.
...
- **ipk / 24.10 до v1.0.** Другой индекс, другая модель доверия, другой инструментарий — это почти
  удваивает поверхность `doctor` ради легаси-линии. Конфиг несёт `format: apk|ipk` с первого дня,
  деревья раздельные, добавление позже чисто аддитивно. До тех пор для 24.10 — подписанные
  россыпью артефакты в релизах (что footstrap уже делает правильно).
...
1. **Принимает ли `apk mkndx` повторный `--sign-key`?** Блокирует всю схему ротации. **Прототип в
   первый день.** Фолбэки описаны выше.
...
3. **Собирается ли apk-tools 3.0.5 под darwin?** Определяет, «работает одинаково на ноутбуке и в CI»
   буквально или «одинаково, через контейнер, на macOS».
4. **Переносимость распаковки хост-`apk` из SDK** по линиям: обёртка vs `.apk.bin` vs бандленный
   загрузчик vs нужен весь `staging_dir/host/lib`. mossdef напоролся на все варианты.
